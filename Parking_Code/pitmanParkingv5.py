...
#this is how we connect to the data base and the name of the database 
client = pymongo.MongoClient('mongodb+srv://sc_delaEmi:u2JsEd0nzYssgaMd@cluster0.8qczawe.mongodb.net/test')
db = client["test"]

garage = input("You are being connected with Pitman Parking! What floor would you like to edit? (Ex. Level_1)\n")
...
class garage:
    ...
    #Gets information about the database and set the garage info to match that
    def populate(self, records):
        levelNumber = 'Level Number' #Removes duplication of literal
        self.floors = 0
        for row in records:
            if (row[levelNumber] > self.floors):
                self.floors = row[levelNumber]

        [self.floorList.append([]) for i in range(self.floors)]

        for row in records:
            newSpot = spot(row['Space ID'], row['Space Occupied'], row[levelNumber], row['Space Number'])
            self.floorList[row[levelNumber] - 1].append(newSpot)
            self.spots += 1
